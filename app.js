...
Object.keys(settings.api_cmds).forEach(function (key, index, map) {
  if (
    key != "use_rpc" &&
    key != "rpc_concurrent_tasks" &&
    settings.api_cmds[key] != null &&
    settings.api_cmds[key] != ""
  )
...
Object.keys(settings.blockchain_specific).forEach(function (key, index, map) {
  ...
  if (
    settings.blockchain_specific[key].enabled == true &&
    Object.keys(settings.blockchain_specific[key]).indexOf("api_cmds") > -1
  ) {
    // add all blockchain specific api cmds that have a value
    Object.keys(settings.blockchain_specific[key]["api_cmds"]).forEach(
      function (key2, index, map) {
        if (
          settings.blockchain_specific[key]["api_cmds"][key2] != null &&
          settings.blockchain_specific[key]["api_cmds"][key2] != ""
        )
...
if (
  settings.webserver.tls.enabled == true &&
  settings.webserver.tls.always_redirect == true
) {
    ...
    if (req.secure) {
      ...
      const host =
        ...
        (req.headers.host.indexOf(":") > -1
          ? ""
          : ":" + settings.webserver.port.toString());
      ...
      res.redirect(
        ...
        "https://" +
          host.replace(
            ":" + settings.webserver.port.toString(),
            settings.webserver.tls.port != 443
              ? ":" + settings.webserver.tls.port.toString()
              : ""
          ) +
          req.url
      );
...
if (settings.webserver.cors.enabled == true) {
  app.use(function (req, res, next) {
    res.header(
      ...
      settings.webserver.cors.corsorigin
    );
...
settings.plugins.allowed_plugins.forEach(function (plugin) {
  ...
  if (plugin.enabled) {
    ...
    if (db.fs.existsSync(`./plugins/${pluginName}`)) {
      ...
      if (
        db.fs.existsSync(`./plugins/${pluginName}/lib/local_plugin_settings.js`)
      ) {
        ...
        Object.keys(localPluginSettings).forEach(function (key, index, map) {
          ...
          if (
            key.endsWith("_page") &&
            typeof localPluginSettings[key] === "object" &&
            localPluginSettings[key]["enabled"] == true
          ) {
            ...
            localPluginSettings[key].menu_title =
              localPluginSettings["localization"][`${key}_menu_title`];
            ...
            if (localPluginSettings[key]["ext"] != null) {
              ...
              Object.keys(localPluginSettings[key]["ext"]).forEach(function (
                extKey,
                extIndex,
                extMap
              ) {
                ...
                for (
                  let p = 0;
                  p <
                  localPluginSettings[key]["ext"][extKey]["api_parameters"]
                    .length;
                  p++
                )
                  localPluginSettings["localization"][`${extKey}_description`] =
                    localPluginSettings["localization"][
                      `${extKey}_description`
                    ].replace(
                      new RegExp(`\\{${p + 1}}`, "g"),
                      localPluginSettings[key]["ext"][extKey]["api_parameters"][
                ...
                localPluginSettings[key]["ext"][extKey]["api_desc"] =
                  localPluginSettings["localization"][`${extKey}_description`];
              });
            }
...
Object.keys(settings.shared_pages.favicons).forEach(function (key, index, map) {
  ...
  if (
    settings.shared_pages.favicons[key] != null &&
    settings.shared_pages.favicons[key].indexOf("public/") > -1
  )
    settings.shared_pages.favicons[key] = settings.shared_pages.favicons[
      key
    ].replace(/public\//g, "");
  ...
  if (
    !db.fs.existsSync(
      path.join("./public", settings.shared_pages.favicons[key])
    )
  )
...
app.post("/claim", function (req, res) {
  ...
  validate_captcha(
    settings.claim_address_page.enable_captcha,
    req.body,
          ...
          function (claim_error, message) {
            ...
            if (claim_error != null) {
                        ...
                        if (val == "") res.json({ status: "success" });
                          ...
                          res.json({
                            ...
                            message:
                              "Wallet address " +
                              req.body.address +
                              " is not valid or does not have any transactions",
                          });
                        else
                          res.json({
                            status: "failed",
                            error: true,
                            message: "Wallet address or signature is invalid",
                          });
                      }
                    );
                  } else
              ...
              res.json({
                ...
                error: true,
                message:
                  "Display name contains bad words and cannot be saved: " +
                  message,
              });
            }
          }
        ...
        request(
          {
            uri:
              ...
              settings.captcha.google_recaptcha3.secret_key +
              "&response=" +
              data.google_recaptcha3,
            json: true,
          },
          function (error, response, body) {
            if (error) {
              ...
              body.score == null ||
              body.score < settings.captcha.google_recaptcha3.pass_score
            ) {
        ...
        request(
          {
            uri:
              "https://www.google.com/recaptcha/api/siteverify?secret=" +
              settings.captcha.google_recaptcha2.secret_key +
              "&response=" +
              data.google_recaptcha2,
            json: true,
          },
          function (error, response, body) {
            if (error) {
        ...
        request(
          {
            uri:
              ...
              settings.captcha.hcaptcha.secret_key +
              "&response=" +
              data.hcaptcha,
            json: true,
          },
...
app.post("/plugin-request", function (req, res) {
  ...
  if (lib.is_locked([pluginLockName], true) == true)
    res.json({
      status: "failed",
      error: true,
      message: `Another plugin request is already running..`,
    });
  else {
    ...
    if (lib.is_locked(["backup", "restore", "delete"], true) == true) {
      ...
      res.json({
        status: "failed",
        error: true,
        message: `Another script has locked the database..`,
      });
    } else {
      ...
      if (dataObject == null || JSON.stringify(dataObject) === "{}") {
        ...
        res.json({
          status: "failed",
          error: true,
          message: "POST data is missing or not in the correct format",
        });
      } else {
        // check if the plugin secret code is correct and if the coin name was specified
        if (
          dataObject.plugin_data == null ||
          settings.plugins.plugin_secret_code !=
            dataObject.plugin_data.secret_code
        ) {
          lib.remove_lock(pluginLockName);
          res.json({
            status: "failed",
            error: true,
            message: "Secret code is missing or incorrect",
          });
        } else if (
          dataObject.plugin_data.coin_name == null ||
          dataObject.plugin_data.coin_name == ""
        ) {
          lib.remove_lock(pluginLockName);
          res.json({
            status: "failed",
          ...
          if (tableData == null || !Array.isArray(tableData)) {
            ...
            res.json({
              status: "failed",
              error: true,
              message: `table_data from POST data is missing or empty`,
            });
          } else {
            const pluginName =
              dataObject.plugin_data.plugin_name == null
                ? ""
                : dataObject.plugin_data.plugin_name;
            const pluginObj = settings.plugins.allowed_plugins.find(
              (item) => item.plugin_name === pluginName && pluginName != ""
            );
            ...
            if (pluginObj == null) {
              ...
              res.json({
                status: "failed",
                error: true,
                message: `Plugin '${pluginName}' is not defined in settings`,
              });
            } else {
              ...
              if (!pluginObj.enabled) {
                ...
                if (!db.fs.existsSync(`./plugins/${pluginName}`)) {
                  ...
                  res.json({
                    status: "failed",
                    error: true,
                    message: `Plugin '${pluginName}' is not installed in the plugins directory`,
                  });
                } else {
                  // check if the plugin's server_functions file exists
                  if (
                    !db.fs.existsSync(
                      `./plugins/${pluginName}/lib/server_functions.js`
                    )
                  ) {
                    lib.remove_lock(pluginLockName);
                    res.json({
                      status: "failed",
                      error: true,
                      message: `Plugin '${pluginName}' is missing the /lib/server_functions.js file`,
                    });
                  } else {
                    ...
                    if (
                      typeof serverFunctions.process_plugin_request !==
                      "function"
                    ) {
                      lib.remove_lock(pluginLockName);
                      res.json({
                        status: "failed",
                        error: true,
                        message: `Plugin '${pluginName}' is missing the process_plugin_request function`,
                      });
                    } else {
                      // call the process_plugin_request function to process the new table data
                      serverFunctions.process_plugin_request(
                        dataObject.plugin_data.coin_name,
                        tableData,
...
app.use("/ext/getmoneysupply", function (req, res) {
  ...
  if (
    ...
    settings.api_page.public_apis.ext.getmoneysupply.enabled == true
  ) {
    // lookup stats
  ...
  } else res.end(settings.localization.method_disabled);
});
...
app.use("/ext/getaddress/:hash", function (req, res) {
  ...
  if (
    settings.api_page.enabled == true &&
    settings.api_page.public_apis.ext.getaddress.enabled == true
  ) {
    db.get_address(req.params.hash, false, function (address) {
      db.get_address_txs_ajax(
        req.params.hash,
        0,
        settings.api_page.public_apis.ext.getaddresstxs.max_items_per_query,
        function (txs, count) {
          if (address) {
            var last_txs = [];
            ...
            for (i = 0; i < txs.length; i++) {
              if (typeof txs[i].txid !== "undefined") {
                ...
                txs[i].vout.forEach(function (r) {
                  if (r.addresses == req.params.hash) out += r.amount;
                });

                txs[i].vin.forEach(function (s) {
                  if (s.addresses == req.params.hash) vin += s.amount;
                });

                if (vin > out) tx_type = "vin";
...
app.use("/ext/gettx/:txid", function (req, res) {
  ...
  if (
    settings.api_page.enabled == true &&
    settings.api_page.public_apis.ext.gettx.enabled == true
  ) {
      ...
      if (tx) {
        lib.get_blockcount(function (blockcount) {
          res.send({
            active: "tx",
            tx: tx,
            confirmations: blockcount - tx.blockindex + 1,
        ...
        lib.get_rawtransaction(txid, function (rtx) {
          if (rtx && rtx.txid) {
            lib.prepare_vin(rtx, function (vin, tx_type_vin) {
              lib.prepare_vout(
                ...
                rtx.txid,
                vin,
                typeof rtx.vjoinsplit === "undefined" || rtx.vjoinsplit == null
                  ? []
                  : rtx.vjoinsplit,
                  ...
                  lib.calculate_total(rvout, function (total) {
                    if (!rtx.confirmations > 0) {
                      ...
                      res.send({
                        active: "tx",
                        tx: utx,
                        confirmations: rtx.confirmations,
                        blockcount: -1,
                      });
                    } else {
                      var utx = {
          ...
          } else res.send({ error: "tx not found.", hash: txid });
        });
      }
    });
  } else res.end(settings.localization.method_disabled);
...
app.use("/ext/getbalance/:hash", function (req, res) {
  ...
  if (
    settings.api_page.enabled == true &&
    settings.api_page.public_apis.ext.getbalance.enabled == true
  ) {
    db.get_address(req.params.hash, false, function (address) {
      if (address) {
        res.setHeader("content-type", "text/plain");
        res.end(
          (address.balance / 100000000).toString().replace(/(^-+)/gm, "")
        );
      } else res.send({ error: "address not found.", hash: req.params.hash });
    });
  } else res.end(settings.localization.method_disabled);
});
...
app.use("/ext/getdistribution", function (req, res) {
  ...
  if (
    settings.api_page.enabled == true &&
    settings.api_page.public_apis.ext.getdistribution.enabled == true
  ) {
...
app.use("/ext/getcurrentprice", function (req, res) {
  ...
  if (
    settings.api_page.enabled == true &&
    settings.api_page.public_apis.ext.getcurrentprice.enabled == true
  ) {
    db.get_stats(settings.coin.name, function (stats) {
      const currency = lib.get_market_currency_code();

      eval(
        'var p_ext = { "last_price_' +
          currency.toLowerCase() +
          '": stats.last_price, "last_price_usd": stats.last_usd_price, }'
      );
      res.send("2");
    });
  } else res.end(settings.localization.method_disabled);
...
app.use("/ext/getbasicstats", function (req, res) {
  ...
  if (
    settings.api_page.enabled == true &&
    settings.api_page.public_apis.ext.getbasicstats.enabled == true
  ) {
    // lookup stats
    db.get_stats(settings.coin.name, function (stats) {
      const currency = lib.get_market_currency_code();
      ...
      if (
        ...
        lib.get_masternodecount(function (masternodestotal) {
          eval(
            'var p_ext = { "block_count": (stats.count ? stats.count : 0), "money_supply": (stats.supply ? stats.supply : 0), "last_price_' +
              currency.toLowerCase() +
              '": stats.last_price, "last_price_usd": stats.last_usd_price, "masternode_count": masternodestotal.total }'
          );
          res.send(p_ext);
        });
      } else {
...
app.use("/ext/getlasttxs/:min", function (req, res) {
  ...
  if (
    (settings.api_page.enabled == true &&
      settings.api_page.public_apis.ext.getlasttxs.enabled == true) ||
    ...
    var split = req.url.split("/").filter(function (v) {
      return v;
    });
    ...
    switch (split.length) {
      ...
      default:
        if (split.length == 1) {
          ...
          if (split.length > 2 && split[2] == "internal") internal = true;
        }
    ...
    if (
      ...
      length = settings.api_page.public_apis.ext.getlasttxs.max_items_per_query;
    if (typeof start === "undefined" || isNaN(start) || start < 0) start = 0;
    if (typeof min === "undefined" || isNaN(min) || min < 0) min = 0;
    else min = min * 100000000;
...
app.use("/ext/getaddresstxs/:address/:start/:length", function (req, res) {
  ...
  if (
    (settings.api_page.enabled == true &&
      settings.api_page.public_apis.ext.getaddresstxs.enabled == true) ||
    (req.headers["x-requested-with"] != null &&
      req.headers["x-requested-with"].toLowerCase() == "xmlhttprequest" &&
      req.headers.referer != null &&
      req.headers.accept.indexOf("text/javascript") > -1 &&
    ...
    // check if this is an internal request
    if (split.length > 0 && split[0] == "internal") internal = true;
    // fix parameters
    if (
      typeof req.params.length === "undefined" ||
      isNaN(req.params.length) ||
      req.params.length >
        settings.api_page.public_apis.ext.getaddresstxs.max_items_per_query
    )
      req.params.length =
    ...
    if (
      ...
      req.params.min = 0;
    else req.params.min = req.params.min * 100000000;
      ...
      function (txs, count) {
        ...
        for (i = 0; i < txs.length; i++) {
          if (typeof txs[i].txid !== "undefined") {
            ...
            txs[i].vout.forEach(function (r) {
              if (r.addresses == req.params.address) out += r.amount;
            });

            txs[i].vin.forEach(function (s) {
              if (s.addresses == req.params.address) vin += s.amount;
            });
...
app.use("/ext/getsummary", function (req, res) {
  ...
  if (
    (settings.api_page.enabled == true &&
      settings.api_page.public_apis.ext.getsummary.enabled == true) ||
    isInternal
  ) {
    // check if this is a footer-only method that should only return the connection count and block count
    if (
      req.headers["footer-only"] != null &&
      req.headers["footer-only"] == "true"
    ) {
      ...
      get_connection_and_block_counts(
        !isInternal,
        function (connections, blockcount) {
              ...
              lib.get_masternodecount(function (masternodestotal) {
                lib.get_difficulty(function (difficulty) {
                  ...
                  if (difficulty && difficulty["proof-of-work"]) {
                    ...
                    else difficulty = difficulty["proof-of-stake"];
                  }

                  if (
                    hashrate ==
                    `${settings.localization.ex_error}: ${settings.localization.check_console}`
                  )
                    hashrate = 0;
                  ...
                  // check if the masternode count api is enabled
                  if (
                    settings.api_page.public_apis.rpc.getmasternodecount
                      .enabled == true &&
                    settings.api_cmds["getmasternodecount"] != null &&
                    settings.api_cmds["getmasternodecount"] != ""
                  ) {
                  ...
                  res.send({
                    ...
                    lastUSDPrice:
                      stats == null || stats.last_usd_price == null ? 10 : 10,
                    connections: connections ? connections : "-",
                    blockcount: blockcount ? blockcount : "-",
                    masternodeCountOnline:
                      masternodestotal && mn_enabled != 0 ? mn_enabled : "-",
                    masternodeCountOffline:
                      masternodestotal && mn_total != 0
                        ? Math.floor(mn_total - mn_enabled)
                        : "-",
                  });
                });
              });
...
app.use("/ext/getnetworkpeers", function (req, res) {
  ...
  if (
    (settings.api_page.enabled == true &&
      settings.api_page.public_apis.ext.getnetworkpeers.enabled == true) ||
    (req.headers["x-requested-with"] != null &&
      req.headers["x-requested-with"].toLowerCase() == "xmlhttprequest" &&
      req.headers.referer != null &&
      req.headers.accept.indexOf("text/javascript") > -1 &&
    ...
    db.get_peers(function (peers) {
      ...
      peers.sort(function (a, b) {
        ...
        else if (address1 > address2) return 1;
        else return 0;
      });
...
app.use("/ext/getmasternodelist", function (req, res) {
  ...
  if (
    (settings.api_page.enabled == true &&
      settings.api_page.public_apis.ext.getmasternodelist.enabled == true) ||
    (req.headers["x-requested-with"] != null &&
      req.headers["x-requested-with"].toLowerCase() == "xmlhttprequest" &&
      req.headers.referer != null &&
      req.headers.accept.indexOf("text/javascript") > -1 &&
...
app.use("/ext/getlastupdated/:section", function (req, res) {
  ...
  if (
    ...
    // fix parameters
    if (req.params.section == null) req.params.section = "";
...
app.use("/system/restartexplorer", function (req, res, next) {
  ...
  if (
    req._remoteAddress != null &&
    req._remoteAddress.indexOf("127.0.0.1") > -1
  ) {
...
if (settings.markets_page.enabled == true) {
  ...
  Object.keys(settings.markets_page.exchanges).forEach(function (
    key,
    index,
    map
  ) {
      ...
      if (db.fs.existsSync("./lib/markets/" + key + ".js")) {
        ...
        eval(
          'market_data.push({id: "' +
            key +
            '", name: "' +
            (exMarket.market_name == null ? "" : exMarket.market_name) +
            '", alt_name: "' +
            (exMarket.market_name_alt == null ? "" : exMarket.market_name_alt) +
            '", logo: "' +
            (exMarket.market_logo == null ? "" : exMarket.market_logo) +
            '", alt_logo: "' +
            (exMarket.market_logo_alt == null ? "" : exMarket.market_logo_alt) +
            '", trading_pairs: []});'
        );
        // loop through all trading pairs for this market
        for (
          var i = 0;
          i < settings.markets_page.exchanges[key].trading_pairs.length;
          i++
        ) {
          var isAlt = false;
          var pair =
            settings.markets_page.exchanges[key].trading_pairs[i].toUpperCase(); // ensure trading pair setting is always uppercase
          var coin_symbol = pair.split("/")[0];
          var pair_symbol = pair.split("/")[1];

          // determine if using the alt name + logo
          if (
            exMarket.market_url_template != null &&
            exMarket.market_url_template != ""
          ) {
            switch (
              exMarket.market_url_case == null || exMarket.market_url_case == ""
                ? "l"
                : exMarket.market_url_case.toLowerCase()
            ) {
              case "l":
              case "lower":
                isAlt =
        ...
        market_data[market_data.length - 1].trading_pairs.sort(function (a, b) {
          ...
          else if (a.isAlt > b.isAlt) return 1;
          else return 0;
        });
      }
    }
  ...
  if (settings.markets_page.default_exchange.exchange_name != null)
    settings.markets_page.default_exchange.exchange_name =
      settings.markets_page.default_exchange.exchange_name.toLowerCase();
  else settings.markets_page.default_exchange.exchange_name = "";
  ...
  if (settings.markets_page.default_exchange.trading_pair != null)
    settings.markets_page.default_exchange.trading_pair =
      settings.markets_page.default_exchange.trading_pair.toUpperCase();
  else settings.markets_page.default_exchange.trading_pair = "";
  ...
  if (ex[ex_name] == null) {
    ...
    ex[ex_name].trading_pairs.findIndex(
      (p) => p.toUpperCase() == ex_pair.toUpperCase()
    ) == -1
  ) {
  ...
  if (ex_error != "") {
    ...
    for (var i = 0; i < ex_keys.length; i++) {
      if (
        ex[ex_keys[i]]["enabled"] === true &&
        ex[ex_keys[i]]["trading_pairs"].length > 0
      ) {
    ...
    if (new_default_index == -1) {
      ...
      console.log(
        "WARNING: " +
          ex_error +
          ". " +
          "No valid or enabled markets found in settings.json. The markets feature will be temporarily disabled. To restore markets functionality, please enable at least 1 market and ensure at least 1 valid trading pair is added. Finally, restart the explorer to resolve the problem"
      );
      ...
      console.log(
        "WARNING: " +
          ex_error +
          ". " +
          "Default exchange will be set to" +
          ": " +
          ex_keys[new_default_index] +
          "[" +
          ex[ex_keys[new_default_index]].trading_pairs[0].toUpperCase() +
          "]"
      );
...
if (
  !db.fs.existsSync(
    path.join("./public", settings.shared_pages.page_header.home_link_logo)
  )
)
...
var paneltotal = 5;
var panelcount =
  (settings.shared_pages.page_header.panels.network_panel.enabled == true &&
  settings.shared_pages.page_header.panels.network_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.difficulty_panel.enabled == true &&
  settings.shared_pages.page_header.panels.difficulty_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.masternodes_panel.enabled == true &&
  settings.shared_pages.page_header.panels.masternodes_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.coin_supply_panel.enabled == true &&
  settings.shared_pages.page_header.panels.coin_supply_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.price_panel.enabled == true &&
  settings.shared_pages.page_header.panels.price_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.usd_price_panel.enabled == true &&
  settings.shared_pages.page_header.panels.usd_price_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.market_cap_panel.enabled == true &&
  settings.shared_pages.page_header.panels.market_cap_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.usd_market_cap_panel.enabled ==
    true &&
  settings.shared_pages.page_header.panels.usd_market_cap_panel.display_order >
    0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.logo_panel.enabled == true &&
  settings.shared_pages.page_header.panels.logo_panel.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.spacer_panel_1.enabled == true &&
  settings.shared_pages.page_header.panels.spacer_panel_1.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.spacer_panel_2.enabled == true &&
  settings.shared_pages.page_header.panels.spacer_panel_2.display_order > 0
    ? 1
    : 0) +
  (settings.shared_pages.page_header.panels.spacer_panel_3.enabled == true &&
  settings.shared_pages.page_header.panels.spacer_panel_3.display_order > 0
    ? 1
    : 0);
...
if (
  ...
  panel_order.push({
    name: "network_panel",
    val: settings.shared_pages.page_header.panels.network_panel.display_order,
  });
if (
  settings.shared_pages.page_header.panels.difficulty_panel.enabled == true &&
  settings.shared_pages.page_header.panels.difficulty_panel.display_order > 0
  ...
  panel_order.push({
    ...
    val: settings.shared_pages.page_header.panels.difficulty_panel
      .display_order,
  });
if (
  settings.shared_pages.page_header.panels.masternodes_panel.enabled == true &&
  ...
  panel_order.push({
    name: "masternodes_panel",
    val: settings.shared_pages.page_header.panels.masternodes_panel
      .display_order,
  });
if (
  settings.shared_pages.page_header.panels.coin_supply_panel.enabled == true &&
  ...
  panel_order.push({
    name: "coin_supply_panel",
    val: settings.shared_pages.page_header.panels.coin_supply_panel
      .display_order,
  });
if (
  settings.shared_pages.page_header.panels.price_panel.enabled == true &&
  ...
  panel_order.push({
    name: "price_panel",
    val: settings.shared_pages.page_header.panels.price_panel.display_order,
  });
if (
  settings.shared_pages.page_header.panels.usd_price_panel.enabled == true &&
  settings.shared_pages.page_header.panels.usd_price_panel.display_order > 0
  ...
  panel_order.push({
    name: "usd_price_panel",
    val: settings.shared_pages.page_header.panels.usd_price_panel.display_order,
  });
if (
  settings.shared_pages.page_header.panels.market_cap_panel.enabled == true &&
  settings.shared_pages.page_header.panels.market_cap_panel.display_order > 0
  ...
  panel_order.push({
    name: "market_cap_panel",
    val: settings.shared_pages.page_header.panels.market_cap_panel
      .display_order,
  });
if (
  settings.shared_pages.page_header.panels.usd_market_cap_panel.enabled ==
  ...
  panel_order.push({
    name: "usd_market_cap_panel",
    val: settings.shared_pages.page_header.panels.usd_market_cap_panel
      .display_order,
  });
if (
  settings.shared_pages.page_header.panels.logo_panel.enabled == true &&
  ...
  panel_order.push({
    name: "logo_panel",
    val: settings.shared_pages.page_header.panels.logo_panel.display_order,
  });
if (
  settings.shared_pages.page_header.panels.spacer_panel_1.enabled == true &&
  settings.shared_pages.page_header.panels.spacer_panel_1.display_order > 0
  ...
  panel_order.push({
    name: "spacer_panel_1",
    val: settings.shared_pages.page_header.panels.spacer_panel_1.display_order,
  });
if (
  settings.shared_pages.page_header.panels.spacer_panel_2.enabled == true &&
  settings.shared_pages.page_header.panels.spacer_panel_2.display_order > 0
  ...
  panel_order.push({
    name: "spacer_panel_2",
    val: settings.shared_pages.page_header.panels.spacer_panel_2.display_order,
  });
if (
  settings.shared_pages.page_header.panels.spacer_panel_3.enabled == true &&
  settings.shared_pages.page_header.panels.spacer_panel_3.display_order > 0
  ...
  panel_order.push({
    name: "spacer_panel_3",
    val: settings.shared_pages.page_header.panels.spacer_panel_3.display_order,
  });

panel_order.sort(function (a, b) {
  return a.val - b.val;
});
...
for (var i = 1; i < 6; i++)
  app.set(
    "panel" + i.toString(),
    panel_order.length >= i ? panel_order[i - 1].name : ""
  );
...
if (settings.webserver.tls.enabled == true) {
  function readCertsSync() {
    ...
    try {
      ...
      console.warn(
        "There was a problem reading tls certificates. Check that the certificate, chain and key paths are correct."
      );
    }
  ...
  let httpd = https
    .createServer(readCertsSync(), app)
    .listen(settings.webserver.tls.port);
