...
module.exports = {
  ...
  wiii_get: function(namespace,key, cb) {
    var cmd = prepareRpcCommand(settings.api_cmds.wiii_get, [namespace,key]);
      ...
      if (settings.api_cmds.use_rpc) {
        rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
          ...
          if (response == 'There was an error. Check your console.')
            ...
            //return cb(response.replace(/\\/g, ''))
        });
      } else {
        var uri = base_url + 'wiii_get?namespace=' + namespace + ',key=' + key;
