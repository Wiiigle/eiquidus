...
const messageVerifier = require('./message_verifier');
...
module.exports = {
  ...
  wg: function(namespace,key, cb) {
    var cmd = prepareRpcCommand(settings.api_cmds.wg, [namespace,key]);
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        var uri = base_url + 'wg?n=' + namespace + ',k=' + key;
  ...
  verify_message: function(address, signature, message, cb) {
    const verifyLocally = () => cb(messageVerifier.verifyMessage(address, signature, message));
    var cmd = prepareRpcCommand(settings.api_cmds.verifymessage, [address, signature, message]);

    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
          // Core's verifymessage only supports legacy P2PKH addresses. Fall
          // back to local verification for wrapped/native SegWit signatures.
          if (response === true)
            return cb(response);
          else
