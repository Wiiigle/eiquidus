  ...
  request(
    ...
    function (error, response, body) {
      if (error) return cb(error, []);
        ...
        body["status"] != null &&
        body["status"]["error_message"] != null &&
  ...
  request(
    {
      uri:
        base_url +
        `simple/price?ids=${id.toLowerCase()}&vs_currencies=usd${
        ...
        try {
          if (market_array != null) {
            ...
            if (currency != null && currency != "") {
              ...
              const base_index = market_array.findIndex(
              ...
              if (base_index > -1) {
                ...
                api_market =
                  body[market_array[base_index].coingecko_id.toLowerCase()];
              }
            }

            // loop through all api object keys
            Object.keys(body).forEach(function (key, index, map) {
              const market_index = market_array.findIndex(
              ...
              if (market_index > -1) {
                ...
                if (
                  currency != null &&
                  currency != "" &&
                  body[key][currency.toLowerCase()] != null
                ) {
                  // calculate the market price
                  last_price +=
                    market_array[market_index].last_price *
                    (currency == null || currency == ""
                      ? 0
                      : body[key][currency.toLowerCase()]);
                } else {
                ...
                last_usd_price +=
                  market_array[market_index].last_price * body[key]["usd"];
            ...
            return cb(
              ...
              currency == null || currency == ""
                ? 0
                : body[id.toLowerCase()][currency.toLowerCase()],
              body[id.toLowerCase()]["usd"]
            );
          }
        } catch (err) {
...
module.exports = {
  ...
  get_avg_market_prices: function (id, currency, market_array, api_key, cb) {
    get_simple_price(
      ...
      market_array,
      api_key,
      function (err, last_price, last_usd) {
        if (last_price == null)
          console.log(
            `Error: "${currency}" is not a valid coingecko api currency`
          );
