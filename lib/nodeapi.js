var onode = require('./node'),
    ...
    wgResponse = require('./wg-response'),
...
module.exports = function() {
  function express_app() {
    ...
    app.get('*', hasAccess, function(req, res) {
      ...
      client.cmd(command, function(err, response) {
        if (err) {
          ...
          // WG keys ending in .json or .txt are raw content endpoints.
          // Other RPC methods and WG keys retain the legacy response behaviour.
          if (method == 'wg' && sendWgResponse(res, query_parameters.k, response))
            return;

          if (typeof response === 'object'){
            //res.json({}); 
            let strIn=JSON.stringify(response);
            //res.JSON.parse(response);
            //let str=JSON.parse(response);
            let strOut=strIn.replace(/\\/g, '');
            let startPos=strOut.indexOf("name");
            startPos=startPos-2;
            strOut=strOut.substring(startPos);
            startPos=strOut.lastIndexOf("txid")-3;
            strOut=strOut.substring(0,startPos);
            res.end(strOut);
          }
          else {
    ...
    function signerAddressFromTx(tx) {
      if (tx == null || !Array.isArray(tx.vin))
        return null;

      for (var i = 0; i < tx.vin.length; i++) {
        var address = tx.vin[i].addresses || tx.vin[i].address;
        if (Array.isArray(address))
          address = address[0];
        if (address && address != 'coinbase' && address != 'unknown_address' && address != 'hidden_address')
          return address;
      }
    ...
    function sendWgResponse(res, key, response) {
      if (!key || (!key.toLowerCase().endsWith('.json') && !key.toLowerCase().endsWith('.txt')))
        return false;

      var txid = wgResponse.findMetadata(response, ['txid', 'tx_hash', 'transaction']);
      if (!txid) {
        wgResponse.sendByKey(res, key, response);
        return true;
      }

      Promise.all([
        Tx.findOne({txid: txid.toString().toLowerCase()}).lean(),
        Stats.findOne({coin: settings.coin.name}).lean()
      ]).then(function(results) {
        var tx = results[0];
        var stats = results[1];
        var signerAddress = signerAddressFromTx(tx);
        var claimPromise = signerAddress
          ? ClaimAddress.findOne({a_id: signerAddress}).lean()
          : Promise.resolve(null);

        return claimPromise.then(function(claim) {
          var blockHeight = tx && tx.blockindex > 0
            ? tx.blockindex
            : wgResponse.findMetadata(response, ['height', 'blockheight', 'block_height']);
          var confirmations = blockHeight && stats && stats.count >= blockHeight
            ? stats.count - blockHeight + 1
            : wgResponse.findMetadata(response, ['confirmations']);

          wgResponse.setMetadataHeaders(res, {
            signerAddress: signerAddress,
            signerName: claim && claim.claim_name,
            signedAt: tx && tx.timestamp,
            txid: txid,
        ...
        console.log('Unable to add W3Sign proof metadata: ' + err.message);
        wgResponse.sendByKey(res, key, response);
      });
    ...
    function specialApiCase(method_name, query_parameters) {
      ...
      switch (method_name) {
        case 'wg':
          for (var parameter in query_parameters) {
            if (query_parameters.hasOwnProperty(parameter) && parameter == 'n')
              params.push(query_parameters[parameter]);
            else if (query_parameters.hasOwnProperty(parameter) && parameter == 'k')
                params.push(query_parameters[parameter]);
          }
          var cmd = prepareRpcCommand(settings.api_cmds.wg, params);
